...
methylationDataList_CHH_pooled <- GRangesList(
...
methylationDataList_CHH_pooled_chr <-lapply(methylationDataList_CHH_pooled, 
                                        filter_scaffolds) 

methylationDataList_CHH_AllSamples_chr_GR<-joinReplicates(methylationDataList_CHH_pooled_chr[[1]], methylationDataList_CHH_pooled_chr[[2]]) 
for (h in 2:3) { #10 samples hence 10-1
  print(h)
  methylationDataList_CHH_AllSamples_chr_GR<-joinReplicates(methylationDataList_CHH_AllSamples_chr_GR, methylationDataList_CHH_pooled_chr[[h+1]])
}

####Define the column names for methylationDataList_Controls_all_single_chr_genotypes
column_names <- rep(names(methylationDataList_CHH_pooled_chr), each=2)
extra_names <- rep(c("meth", "cov"),times=4)
column_extra_names <- paste(column_names,extra_names, sep ="_")


colnames(mcols(methylationDataList_CHH_AllSamples_chr_GR))[2:9] <- column_extra_names
...
methylationDataList_filtered_CHH_AllSamples_chr_GR<- HardFilterCoverage(methylationDataList_CHH_AllSamples_chr_GR,
                                                                           min_cov=8,
                                                                           min_samples = 3)
#202576011
#167706295
...
seqlengths(methylationDataList_filtered_CHH_AllSamples_chr_GR) <- chr_lengths
...
TotalHits_Tile<- TileGenomeToSnippets(methylationDataList_filtered_CHH_AllSamples_chr_GR,
                                      tilesize = 200,
                                      last_tile = TRUE)
...
library(dplyr)
tile200bp_CHH_CX_DF <- data.frame(tile = TotalHits_Tile$tile_index,
                              AA_28dap_pooled_meth = mcols(methylationDataList_filtered_CHH_AllSamples_chr_GR)$AA_28dap_pooled_meth[TotalHits_Tile$meth_index],
                              AA_28dap_pooled_cov = mcols(methylationDataList_filtered_CHH_AllSamples_chr_GR)$AA_28dap_pooled_cov[TotalHits_Tile$meth_index],
                              
                              AA_42dap_H_pooled_meth = mcols(methylationDataList_filtered_CHH_AllSamples_chr_GR)$AA_42dap_H_pooled_meth[TotalHits_Tile$meth_index],
                              AA_42dap_H_pooled_cov = mcols(methylationDataList_filtered_CHH_AllSamples_chr_GR)$AA_42dap_H_pooled_cov[TotalHits_Tile$meth_index],
                              
                              CA_28dap_pooled_meth = mcols(methylationDataList_filtered_CHH_AllSamples_chr_GR)$CA_28dap_pooled_meth[TotalHits_Tile$meth_index],
                              CA_28dap_pooled_cov = mcols(methylationDataList_filtered_CHH_AllSamples_chr_GR)$CA_28dap_pooled_cov[TotalHits_Tile$meth_index],
                              
                              CA_42dap_H_pooled_meth = mcols(methylationDataList_filtered_CHH_AllSamples_chr_GR)$CA_42dap_H_pooled_meth[TotalHits_Tile$meth_index],
                              CA_42dap_H_pooled_cov = mcols(methylationDataList_filtered_CHH_AllSamples_chr_GR)$CA_42dap_H_pooled_cov[TotalHits_Tile$meth_index])
library(data.table)
fwrite(tile200bp_CHH_CX_DF, 
       "/media/rna/VAQUITA/DARREN_WGS_HIGH_HEAT/MethylationPopStruct_1st2nd_Exp/MethylationDistributionPlot/tile200bp_CHH_CX_DF.tsv", 
       sep = "\t")

###load the aggregate dataset
tile200bp_CHH_CX_DF_AGG <- read.delim("/media/rna/VAQUITA/DARREN_WGS_HIGH_HEAT/MethylationPopStruct_1st2nd_Exp/MethylationDistributionPlot/tile200bp_CHH_CX_DF_AGG.tsv")

##count total number of 0s
tile200bp_CHH_CX_DF_AGG %>% summarise(
...
tile200bp_CHH_weightMeth <- tile200bp_CHH_CX_DF_AGG %>% 
...
tile200bp_CHH_weightMeth %>%
  filter(if_any(everything(), is.na)) %>% nrow()

##Omit all NA values, likely come from at least one of 4 samples have 0 coverage
tile200bp_CHH_weightMeth_na_omit <- na.omit(tile200bp_CHH_weightMeth)

write.table(tile200bp_CHH_weightMeth_na_omit,
            "/media/rna/VAQUITA/DARREN_WGS_HIGH_HEAT/MethylationPopStruct_1st2nd_Exp/MethylationDistributionPlot/tile200bp_CHH_weightMeth_na_omit.txt",
            col.names = TRUE, row.names = FALSE, quote = FALSE)

tile200bp_CHH_weightMeth_na_omit_long <- tile200bp_CHH_weightMeth_na_omit %>% 
  pivot_longer(cols = -tile,
               names_to = "Samples",
               values_to = "Weighted_Methylation")
               
ggplot(tile200bp_CHH_weightMeth_na_omit_long, aes(x = Samples, y = Weighted_Methylation, fill = Samples)) +
